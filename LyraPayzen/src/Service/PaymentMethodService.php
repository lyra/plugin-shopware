...
class PaymentMethodService
{
    ...
    public function getPayzenPaymentMethodId(Context $context): ?string
    {
        $criteria = new Criteria();
        $criteria->addFilter(new EqualsFilter('handlerIdentifier', PaymentPayzen::class));

        return $this->paymentRepository->searchIds($criteria, $context)->firstId();
    }

    public function isPayzenPaymentMethodInSalesChannel(SalesChannelContext $salesChannelContext): bool
    {
        $context = $salesChannelContext->getContext();

        $payzenlPaymentMethodId = $this->getPayzenPaymentMethodId($context);
        if (! $payzenlPaymentMethodId) {
            return false;
        }
        ...
        if ($paymentMethods->get($payzenlPaymentMethodId) instanceof PaymentMethodEntity) {
            return true;
        }
