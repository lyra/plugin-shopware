...
class ConfigService
{
    public const SYSTEM_CONFIG_DOMAIN = 'LyraPaymentPayzen.config.';
    ...
    {
        $setting = 'payzen' .  str_replace(' ', '', ucwords(str_replace('_', ' ', $configSetting)));
        switch ($setting) {
            case 'payzenRedirectEnabled':
                $payzenRedirectEnabled = (bool) $this->systemConfigService->get(self::SYSTEM_CONFIG_DOMAIN . $setting, $salesChannelId);
                return ($payzenRedirectEnabled) ? "true" : "false";

            case 'payzenAvailableLanguages':
            case 'payzenPaymentCards':
